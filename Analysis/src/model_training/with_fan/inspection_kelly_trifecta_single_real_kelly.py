...
def inspect_model():
    ...
    month_folders = ['2407', '2408', '2409', '2410', '2411']
    # month_folders = ['2410']
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(15,32)],
        '2411': [f'2411{day:02d}' for day in range(1, 5)],
    }
    ...
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('prob_0_distribution_trifecta_single_test_real.png')
    plt.show()
    print("プロットが 'prob_0_distribution_trifecta_single_test_real.png' として保存されました。")
    ...
    # EV計算のための減算値のリスト
    ev_subtract_values = [25,30,35,40,45,50]
    ...
    # 結果をCSVに保存
    summary_df.to_csv('trifecta_summary_comparison_single_test_real.csv', index=False)
    print("結果の比較を 'trifecta_summary_comparison_single_test_real.csv' として保存しました。")
    ...
    for ev_value in ev_subtract_values:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'revenue_per_date_ev_{ev_value}_test_real.png')
        plt.show()
        print(f"日付ごとの収益を 'revenue_per_date_ev_{ev_value}_test_real.png' として保存しました。")
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'investment_per_date_histogram_ev_{ev_value}_test_real.png')
        plt.show()
        print(f"1日当たりの投資金額のヒストグラムを 'investment_per_date_histogram_ev_{ev_value}_test_real.png' として保存しました。")
        ...
        # 会場ごとの回収率をCSVに保存
        venue_summary.to_csv(f'return_rate_per_venue_ev_{ev_value}_test_real.csv', index=False)
        print(f"会場ごとの回収率を 'return_rate_per_venue_ev_{ev_value}_test_real.csv' として保存しました。")
    # ------------- 追加部分終了 -------------
def process_race(race_id, race_odds, data_boats, total_capital=100000, unit=100, prob_threshold=0.01, ev_subtract=1.4):
    """
    ...
    total_prob = sum(boats_probs.values())
    if total_prob == 0:
        # 確率の合計が0の場合
        columns = ['レースID', 'Boat1', 'Boat2', 'Boat3', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout']
        return pd.DataFrame(columns=columns)
    boats_probs = {k: v / total_prob for k, v in boats_probs.items()}
    ...
    total_investment_per_race = 50 * (1 + total_expected_value)
...
def calculate_trifecta_bets(trifecta_odds_df, data_boats, total_capital=100000, unit=100, prob_threshold=0.01, ev_subtract=1.4):
    """
...
def plot_cumulative_returns(cumulative_returns):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('cumulative_return_comparison_single_test_real.png')
    plt.show()
    print("資金の推移を 'cumulative_return_comparison_single_test_real.png' として保存しました。")
