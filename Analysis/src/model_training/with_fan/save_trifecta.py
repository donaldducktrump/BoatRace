...
# 日付範囲の設定（開始日と終了日を設定）
START_DATE = datetime.strptime('2024-11-06', '%Y-%m-%d')
...
# URLテンプレート
URL_TEMPLATE = 'https://www.boatrace.jp/owpc/pc/race/odds3t?rno={rno}&jcd={jcd}&hd={date}'
...
def get_trifecta_odds_url(date, place_cd, race_no):
    """
    三連単オッズのURLを生成します。
    ...
    Returns:
        str: 三連単オッズのURL
    """
    return URL_TEMPLATE.format(rno=race_no, jcd=place_cd, date=date)

# --- HTML解析関数 ---

def parse_trifecta_odds(html_content):
    """
...
def fetch_trifecta_odds(args, session):
    """
    ...
    url = get_trifecta_odds_url(date_str, place_cd, race_no)
    ...
    try:
        ...
        trifecta_data = parse_trifecta_odds(response.text)
        if not trifecta_data:
            print(f"No trifecta data found for date {date_str}, place_cd {place_cd}, race_no {race_no}")
            return None
        
        df = pd.DataFrame(trifecta_data)
        ...
        print(f"Successfully retrieved data for date {date_str}, place_cd {place_cd}, race_no {race_no}")
        return df
...
def save_trifecta_data():
    ...
    for date in date_range:
        ...
        save_path = os.path.join(save_dir, f'trifecta_{date_str_name}.csv')
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            future_to_args = {executor.submit(fetch_trifecta_odds, args, session): args for args in args_list}
        ...
        if all_data:
            ...
            # 列の順序を調整
            full_df = full_df[['Date', 'JCD', 'Race', 'Boat1', 'Boat2', 'Boat3', 'Odds']]
            # 保存
