...
def inspect_model():
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(15,30,2)]  # 241001 - 241008
    }
    ...
    # 予測
    odds_inv_pred = gbm_odds.predict(X_processed)
    data['odds_inv_pred'] = odds_inv_pred
    group_sum = data.groupby('レースID')['odds_inv_pred'].transform('sum')
    data['odds_inv_pred'] = np.where(group_sum != 0, data['odds_inv_pred'] / group_sum, 0)
    print(data)
    ratio_sum = data.groupby('レースID')['odds_inv_pred'].sum()
    print(ratio_sum)

    data['odds_pred'] = 0.725/data['odds_inv_pred']
    # odds_pred[odds_pred < 1] = 1
    # data['odds_pred'] = odds_pred
    # data.loc[data['艇番'] == 1, 'odds_pred'] = data.loc[data['艇番'] == 1, 'odds_pred'] / 2
    ...
    with open('isotonic_reg_model1.pkl', 'rb') as f:
        loaded_iso_reg = pickle.load(f)

    with open('isotonic_reg_model2.pkl', 'rb') as f:
    ...
    data['prob_odds_product'] = data['prob_0'] * data['odds_pred']

    # # ステップ 2: 期待値が1以上のものにフィルタリング
    data['should_bet'] = data['prob_odds_product'] >= 1

    # # 賭け対象のデータを抽出
    # bet_data = data[data['should_bet']].copy()

    # # ステップ 3: 賭け金額のスケーリング
    # # 賭け金額を100円から1000円の範囲にスケーリング
    # min_bet = 100
    # max_bet = 1000

    # # 最小・最大の期待値を取得
    # min_ev = bet_data['prob_odds_product'].min()
    # max_ev = bet_data['prob_odds_product'].max()

    # # スケーリング関数の定義
    # def scale_bet(ev, min_ev, max_ev, min_bet=100, max_bet=1000):
    #     if max_ev == min_ev:
    #         return max_bet  # 全てのEVが同じ場合、最大賭け金を設定
    #     else:
    #         # 線形スケーリング
    #         return min_bet + (ev - min_ev) / (max_ev - min_ev) * (max_bet - min_bet)

    # # 賭け金額を計算
    # bet_data['bet_amount'] = bet_data['prob_odds_product'].apply(lambda x: scale_bet(x, min_ev, max_ev, min_bet, max_bet))

    # # 賭け金額を整数に変換
    # bet_data['bet_amount'] = bet_data['bet_amount'].astype(int)

    # # ステップ 4: 賭け金額を元のデータフレームに統合
    # data['bet_amount'] = 0  # 初期化
    # data.loc[bet_data.index, 'bet_amount'] = bet_data['bet_amount']

    # # ステップ 5: 正規分布に基づく賭け金額の可視化（オプション）
    # plt.figure(figsize=(10,6))
    # count, bins, ignored = plt.hist(bet_data['prob_odds_product'], bins=30, density=True, alpha=0.6, color='g', label='Data Histogram')

    # import scipy.stats as stats

    # # 正規分布曲線のプロット
    # mu = bet_data['prob_odds_product'].mean()
    # sigma = bet_data['prob_odds_product'].std()
    # xmin, xmax = plt.xlim()
    # x = np.linspace(xmin, xmax, 100)
    # p = stats.norm.pdf(x, mu, sigma)
    # plt.plot(x, p, 'k', linewidth=2, label='Normal Distribution')

    # plt.axvline(x=1, color='r', linestyle='--', label='prob_odds_product = 1')
    # plt.title('Distribution of prob_0 * odds_pred with Normal Fit')
    # plt.xlabel('prob_0 * odds_pred')
    # plt.ylabel('Density')
    # plt.legend()
    # plt.show()
    # # data['bet_amount'] = np.where(data['prob_0'] * data['odds_pred'] > 1, 100, 0)
    ...
    # data['bet_amount'] = 100*data['should_bet'] # 確率が低い場合は掛け金を0にする
