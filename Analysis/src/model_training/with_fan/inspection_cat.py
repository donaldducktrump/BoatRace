...
def inspect_model():
    ...
    try:
        ...
        cat_model.load_model('boatrace_model_catboost.cbm')
    ...
    date_files = {
        ...
        # '2410': [f'2410{day:02d}' for day in range(21, 27)],  # 241020 - 241031
    }
    ...
    for col in categorical_features:
        X[col] = X[col].astype(str)  # CatBoost用にカテゴリカル変数を文字列に変換
    ...
    # CatBoost Pool作成
    test_pool = Pool(X_processed, cat_features=categorical_features)

    # 予測
    y_pred = cat_model.predict_proba(test_pool)
    ...
    total_return_trio = 0
    payout_list_trio = []
    data['prob_0'] = data['prob_0'].fillna(0)
    # レースごとに予測順位を決定
    ...
    for race_id in race_ids:
        ...
        # #複勝の場合
        # if race_data.iloc[0]['prob_0'] > 0.85:
        ...
        # 単勝の場合
        # 正答率の計算
        if race_data.iloc[0]['prob_0'] < 0.4:
            total_bet += 100
            if predicted_first == actual_first:
                correct_predictions += 1
                # オッズ情報の取得
