...
def create_model():
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(15,29,2)],   # 241001 - 241008
    }
    ...
    plt.tight_layout()
    plt.savefig('feature_importance_odds_inv.png')
    plt.show()

    odds_inv_pred = gbm.predict(X_processed)
    ...
    with open('isotonic_reg_model1.pkl', 'wb') as f:
        pickle.dump(iso_reg, f)

    with open('isotonic_reg_model2.pkl', 'wb') as f:
        pickle.dump(iso_reg2, f)

    # モデルの保存
    gbm.save_model('boatrace_odds_model_inv.txt')

    # 特徴量名の保存
    X_processed.columns.to_series().to_csv('feature_names_odds_inv.csv', index=False)
