...
def create_model():
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(15,23)],   # 241001 - 241008
    }
    ...
    for month in month_folders:
        for date in date_files[month]:
            try:
                ...
                if not b_data.empty and not before1min_data.empty and not before_data.empty:
                    # データのマージ
                    data = b_data
    ...
    # モデルの保存
    gbm.save_model('boatrace_odds_model_inv_test.txt')

    # 特徴量名の保存
    X_processed.columns.to_series().to_csv('feature_names_odds_inv_test.csv', index=False)
