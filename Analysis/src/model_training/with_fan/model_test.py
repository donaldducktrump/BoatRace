...
def create_model():
    ...
    date_files = {
        ...
        # '2411': [f'2411{day:02d}' for day in range(1, 5)],   # 241101 - 241104
    }
    ...
    # odds_data に '三連単結果' が含まれていることを確認
    if '三連単結果' not in odds_data.columns:
        print("'三連単結果' カラムが odds_data に存在しません。")
        return

    # '三連単結果' を data にマージ
    data = pd.merge(
        data, 
        odds_data[['レースID', '三連単結果']], 
        on='レースID', 
        how='left'
    )
    ...
    data['target'] = data.apply(target_label, axis=1)
    ...
    # モデルの保存
    gbm.save_model('boatrace_model_first_test.txt')

    # 特徴量名の保存
    X_processed.columns.to_series().to_csv('feature_names_first_test.csv', index=False)
