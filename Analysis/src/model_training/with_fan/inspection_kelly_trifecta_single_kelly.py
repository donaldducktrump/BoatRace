...
def inspect_model():
    ...
    for month in month_folders:
        for date in date_files[month]:
            try:
                ...
                # before1min_dataの読み込みを試行
                try:
                    before1min_data = load_before1min_data(before1min_file)
                    before1min_exists = not before1min_data.empty
                except FileNotFoundError:
                    print(f"before1min_file not found for date: {date}, substituting win_odds and place_odds with win_odds_mean")
                    before1min_exists = False
                    before1min_data = pd.DataFrame()  # 空のDataFrameを作成
                ...
                if not b_data.empty and not k_data.empty and not before_data.empty:
                    ...
                    if before1min_exists:
                        ...
                        # before1min_dataが存在しない場合の処理
                        # win_odds_mean を使用して win_odds と place_odds を補完
                        data['win_odds1min'] = data['win_odds_mean']
                        data['place_odds1min'] = data['place_odds_mean']
    ...
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('prob_0_distribution_trifecta_single_odds_kelly.png')
    plt.show()
    print("プロットが 'prob_0_distribution_trifecta_single_odds_kelly.png' として保存されました。")
    ...
    if 'win_odds1min' not in data.columns:
        data['win_odds1min'] = data['win_odds_mean']
    if 'place_odds1min' not in data.columns:
        data['place_odds1min'] = data['win_odds_mean']
    ...
    # EV計算のための減算値のリスト
    ev_subtract_values = [1,1.5,3,4.5,10]
    ...
    for ev_value in ev_subtract_values:
        ...
        trifecta_bets, total_investment, total_payout, return_rate = calculate_trifecta_bets(
            ...
            total_investment_per_race=1000,
    ...
    # 結果をCSVに保存
    summary_df.to_csv('trifecta_summary_comparison_single_odds_kelly.csv', index=False)
    print("結果の比較を 'trifecta_summary_comparison_single_odds_kelly.csv' として保存しました。")
    ...
    for ev_value in ev_subtract_values:
        ...
        print(trifecta_bets[trifecta_bets['Bet_Amount'] > 0].tail(200))
        # 日付ごとの収益を計算
...
def process_race(race_id, race_odds, data_boats, odds_data, total_investment_per_race, total_capital=100000, unit=100, prob_threshold=0.01, ev_subtract=1.4):
    """
    ...
    Args:
        ...
        odds_data (DataFrame): 三連単結果を含むデータフレーム
        total_investment_per_race (int): 各レースにおける総投資額（円）
    ...
    # 例えば、レース全体の期待値に基づいて投資額を決定する場合:
    total_expected_value = merged_df['EV'].sum()
    total_investment_per_race = 100 * (1 + total_expected_value)

    max_investment = 1000  # 最大投資額（例）
    total_investment_per_race = min(total_investment_per_race, max_investment)


    # Check if any bets remain after filtering
    if merged_df.empty:
        # 賭けがなかった場合
        columns = ['レースID', 'Boat1', 'Boat2', 'Boat3', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout']
        return pd.DataFrame(columns=columns)
    ...
    # ケリー基準に基づくベット額の計算
    merged_df['Kelly_Fraction'] = merged_df.apply(
        lambda row: calculate_kelly_criterion(row['Probability'], row['Odds']), axis=1
    )

    total_kelly_fraction = merged_df['Kelly_Fraction'].sum()

    if total_kelly_fraction > 0:
        # ベット額を計算
        merged_df['Bet_Amount_float'] = (merged_df['Kelly_Fraction'] / total_kelly_fraction) * total_investment_per_race

        # ベット額をスケーリング
        total_bet_amount_float = merged_df['Bet_Amount_float'].sum()
        scaling_factor = total_investment_per_race / total_bet_amount_float
        merged_df['Bet_Amount_float'] *= scaling_factor

        # ベット額を100円単位に丸める
        merged_df['Bet_Amount'] = merged_df['Bet_Amount_float'].apply(round_bet_amount)

        # 投資金額の合計を確認し、差額を調整
        total_bet_amount = merged_df['Bet_Amount'].sum()
        delta = int(total_investment_per_race - total_bet_amount)

        if delta != 0:
            # 小数部分を計算
            merged_df['Fractional_Part'] = merged_df['Bet_Amount_float'] - merged_df['Bet_Amount_float'].astype(int)
            if delta > 0:
                # 残りの金額を小数部分の大きい順に分配
                merged_df = merged_df.sort_values(by='Fractional_Part', ascending=False).reset_index(drop=True)
                for i in range(int(delta / 100)):
                    merged_df.loc[i % len(merged_df), 'Bet_Amount'] += 100
            else:
                # オーバー分を小数部分の小さい順に減額
                delta = -delta
                merged_df = merged_df.sort_values(by='Fractional_Part', ascending=True).reset_index(drop=True)
                for i in range(int(delta / 100)):
                    if merged_df.loc[i % len(merged_df), 'Bet_Amount'] >= 100:
                        merged_df.loc[i % len(merged_df), 'Bet_Amount'] -= 100
            # 一時的な列を削除
            merged_df.drop(columns=['Fractional_Part'], inplace=True)

        # 一時的な列を削除
        merged_df.drop(columns=['Bet_Amount_float'], inplace=True)
    else:
        # ケリー基準に基づく正のベット額がない場合
        columns = ['レースID', 'Boat1', 'Boat2', 'Boat3', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout']
        return pd.DataFrame(columns=columns)

    # 三連単オッズデータから実際の三連単結果を取得
...
def calculate_trifecta_bets(trifecta_odds_df, data_boats, odds_data, total_investment_per_race, total_capital=100000, unit=100, prob_threshold=0.0, ev_subtract=1.4):
    """
    全レースに対して三連単の賭け金額を決定し、回収率や資金の推移を計算します。
    Args:
        trifecta_odds_df (DataFrame): 三連単オッズデータ
        data_boats (DataFrame): 各艇の確率データ
        odds_data (DataFrame): 三連単結果を含むデータフレーム
        total_investment_per_race (int): 各レースにおける総投資額（円）
        total_capital (int): 総資金
        unit (int): 賭け金の単位（円）
        ...
        ev_subtract (float): EV計算の減算値
    Returns:
        tuple: (trifecta_bets DataFrame, total_investment, total_payout, return_rate)
    """
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        futures = {
            executor.submit(
                process_race, race_id, race_odds, data_boats, odds_data, total_investment_per_race, total_capital, unit, prob_threshold, ev_subtract
...
def plot_cumulative_returns(cumulative_returns):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('cumulative_return_comparison_single_odds_kelly.png')
    plt.show()
    print("資金の推移を 'cumulative_return_comparison_single_odds_kelly.png' として保存しました。")
