...
from ...utils.mymodule import (
    load_b_file, load_k_file, preprocess_data, load_before_data, merge_before_data
)
...
def inspect_model():
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(1, 31)],
    }
    ...
    X_processed = pd.concat(
        [X_numeric, X_categorical], axis=1
    )
    ...
    # 予測結果を保存するDataFrameを初期化
    y_pred_df = pd.DataFrame(index=X_processed.index, columns=['prob_0', 'prob_1'])
    ...
    for venue in venues:
        ...
        if os.path.exists(model_filename):
            gbm = lgb.Booster(model_file=model_filename)
        else:
            print(f"Model for venue {venue} not found, skipping.")
            continue

        # 該当会場のデータを抽出
        indices = data[data['会場'] == venue].index
        if indices.empty:
            print(f"No data found for venue {venue}, skipping.")  
            continue
        
        X_venue = X_processed.loc[indices]

        # 予測を実行
        y_pred_venue = gbm.predict(X_venue, num_iteration=gbm.best_iteration)

        # DataFrameに予測結果を保存
        y_pred_venue_df = pd.DataFrame(
            y_pred_venue, index=indices, columns=['prob_0', 'prob_1']
        )
        y_pred_df.loc[indices] = y_pred_venue_df

    # 予測結果をデータに追加
    data['prob_0'] = y_pred_df['prob_0'].astype(float)
    data['prob_1'] = y_pred_df['prob_1'].astype(float)
    ...
    plt.hist(
        data['prob_0'].dropna(), bins=50, color='blue', edgecolor='black', alpha=0.7
    )
    plt.title("予測確率 'prob_0' の分布")
    plt.xlabel("予測確率 (prob_0)")
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('prob_0_distribution_trifecta_venue.png')
    plt.show()
    print("プロットが 'prob_0_distribution_trifecta_venue.png' として保存されました。")
   ...
   # パラメータ設定
    ...
    end_date = datetime.strptime('2024-10-30', '%Y-%m-%d')  # 注意: end_dateがstart_dateより前になっています
...
def plot_results(trifecta_bets, capital_evolution):
    ...
    if capital_evolution.empty or (capital_evolution.shape[0] == 1 and capital_evolution['Capital'].iloc[0] == 100000):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig('cumulative_return_trifecta_venue.png')
        plt.show()
        print("プロットが 'cumulative_return_trifecta_venue.png' として保存されました。")
        ...
        plt.tight_layout()
        plt.savefig('cumulative_return_by_date_trifecta_venue.png')
        plt.show()
        print("日付ごとの累積リターンの推移が 'cumulative_return_by_date_trifecta_venue.png' として保存されました。")
        ...
        if not trifecta_bets.empty:
            ...
            plt.ylabel("度数")
            plt.grid(True)
            plt.savefig('payout_distribution_trifecta_venue.png')
            plt.show()
            ...
            plt.ylabel("度数")
            plt.grid(True)
            plt.savefig('bet_amount_distribution_trifecta_venue.png')
            plt.show()
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig('investment_vs_payout_trifecta_venue.png')
            plt.show()
            ...
            plt.savefig('cumulative_return_by_date_trifecta_venue.png')
            plt.show()
            print("日付を横軸とした累積収益の推移が 'cumulative_return_by_date_trifecta_venue.png' として保存されました。")
        else:
...
def correlation_analysis(trifecta_bets, data):
    ...
    plt.tight_layout()
    plt.savefig('correlation_heatmap_venue.png')
    plt.show()
...
def visualize_conditions(trifecta_bets, data):
    ...
    if 'weather' in merged_data.columns:
        ...
        plt.ylabel('回収率')
        plt.tight_layout()
        plt.savefig('return_rate_by_weather_venue.png')
        plt.show()
    ...
    if '会場' in merged_data.columns:
        ...
        plt.ylabel('回収率')
        plt.tight_layout()
        plt.savefig('return_rate_by_venue_venue.png')
        plt.show()
