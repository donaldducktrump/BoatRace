...
def inspect_model():
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(1,31)],   # 241030 - 241031
    }
    ...
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('prob_0_distribution_trifecta_today.png')
    plt.show()
    print("プロットが 'prob_0_distribution_trifecta_today.png' として保存されました。")
    ...
    base_dir_tricomination = r'C:\Users\kazut\OneDrive\Documents\BoatRace\ML_pred\trio_odds'  # 三連複オッズデータのディレクトリ
    start_date = datetime.strptime('2023-11-01', '%Y-%m-%d')
    end_date = datetime.strptime('2024-10-30', '%Y-%m-%d')  # Adjusted to be after start_date
    total_capital = 100000  # 総資金（円）
    unit = 100  # 賭け金の単位（円）
    prob_threshold = 0  # 賭ける確率の閾値

    # 三連単オッズデータの読み込み
    trifecta_odds_data_df = load_trifecta_odds(base_dir_trifecta, start_date, end_date)

    # 三連複オッズデータの読み込み
    tricomination_odds_data_df = load_tricomination_odds(base_dir_tricomination, start_date, end_date)

    if trifecta_odds_data_df.empty and tricomination_odds_data_df.empty:
    ...
    # 三連単と三連複賭け金と回収金額の計算
    all_bets, total_investment, total_payout, return_rate = calculate_bets(
        trifecta_odds_data_df, 
        tricomination_odds_data_df,  # 三連複オッズデータを追加
        data_boats, 
    ...
    # 資金の推移を計算
    capital_evolution = calculate_capital_evolution(all_bets)

    # 結果の可視化
    plot_results(all_bets, capital_evolution)

    # 各会場ごとの回収率を計算・表示
    calculate_return_rate_by_venue(all_bets)

    # 相関分析と条件別の可視化
    correlation_analysis(all_bets, data)
    visualize_conditions(all_bets, data)
    ...
    if not all_bets.empty:
        ...
        print("\n最初の数レースの詳細:")
        print(all_bets.head(50))
    else:
        # 列を持つ空のDataFrameを表示
        columns = ['レースID', 'Boat1', 'Boat2', 'Boat3', 'Bet_Type', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout']
        empty_df = pd.DataFrame(columns=columns)
        print("\nNo bets were placed. Displaying an empty DataFrame structure:")
        print(empty_df)
...
# 新たに三連複オッズを読み込む関数を追加
def load_tricomination_odds(base_dir, start_date, end_date):
    """
    指定されたディレクトリから三連複オッズデータを読み込み、1つのDataFrameに統合します。
    Args:
        base_dir (str): 三連複オッズデータが保存されているベースディレクトリ
        start_date (datetime): 開始日
        end_date (datetime): 終了日
    Returns:
        DataFrame: 統合された三連複オッズデータ
    """
    all_data = []
    current_date = start_date
    total_days = (end_date - start_date).days + 1
    pbar = tqdm(total=total_days, desc="Loading Tricomination Odds")

    while current_date <= end_date:
        date_str = current_date.strftime('%y%m%d')  # 修正箇所
        yymm = current_date.strftime('%y%m')  # フォルダ名が 'yymm' の場合
        file_name = f'trio_{date_str}.csv'  # 三連複オッズのファイル名
        file_path = os.path.join(base_dir, yymm, file_name)
    ...
    if all_data:
        tricomination_odds_data_df = pd.concat(all_data, ignore_index=True)
        print(f"Total tricomination odds records loaded: {len(tricomination_odds_data_df)}")
        return tricomination_odds_data_df
    else:
        print("No tricomination odds data loaded.")
        return pd.DataFrame()

def calculate_bets(trifecta_odds_df, tricomination_odds_df, data_boats, total_capital=100000, unit=100, prob_threshold=0.0):
    """
    全レースに対して三連単と三連複の賭け金額を決定し、回収率や資金の推移を計算します。
    Args:
        trifecta_odds_df (DataFrame): 三連単オッズのデータ
        tricomination_odds_df (DataFrame): 三連複オッズのデータ
    ...
    Returns:
        ...
        float: 回収率
    """
    race_ids_trifecta = trifecta_odds_df['レースID'].unique()
    race_ids_tricomination = tricomination_odds_df['レースID'].unique()
    all_race_ids = set(race_ids_trifecta).union(set(race_ids_tricomination))
    all_trifecta_bets = []
    all_tricomination_bets = []

    # trifecta_odds_df を 'レースID' でグループ化
    grouped_trifecta_odds = trifecta_odds_df.groupby('レースID')
    # tricomination_odds_df を 'レースID'でグループ化
    grouped_tricomination_odds = tricomination_odds_df.groupby('レースID')

    race_odds_list_trifecta = []
    race_odds_list_tricomination = []

    total_groups_trifecta = len(grouped_trifecta_odds)
    total_groups_tricomination = len(grouped_tricomination_odds)

    with tqdm(total=total_groups_trifecta + total_groups_tricomination, desc="Grouping Odds") as pbar_group:
        for race_id, race_odds in grouped_trifecta_odds:
            race_odds_list_trifecta.append((race_id, race_odds))
            pbar_group.update(1)
        for race_id, race_odds in grouped_tricomination_odds:
            race_odds_list_tricomination.append((race_id, race_odds))
            pbar_group.update(1)
    ...
    with ThreadPoolExecutor(max_workers=1000) as executor:
        ...
        futures_trifecta = {
            executor.submit(
                process_race_trifecta, race_id, race_odds, data_boats, total_capital, unit, prob_threshold
            ): race_id for race_id, race_odds in race_odds_list_trifecta
        }

        # 三連複の処理
        futures_tricomination = {
            executor.submit(
                process_race_tricomination, race_id, race_odds, data_boats, total_capital, unit, prob_threshold
            ): race_id for race_id, race_odds in race_odds_list_tricomination
        }

        # プログレスバーの設定（処理中の進行状況と残り時間を表示）
        with tqdm(total=len(futures_trifecta) + len(futures_tricomination), desc="Processing Bets") as pbar_bets:
            for future in as_completed(list(futures_trifecta.keys()) + list(futures_tricomination.keys())):
                race_id = futures_trifecta.get(future, futures_tricomination.get(future))
                try:
                    bet = future.result()
                    if bet is not None:
                        for b in bet:  # Iterate over each bet in the list
                            if b['Bet_Type'] == 'trifecta':
                                all_trifecta_bets.append(b)
                            elif b['Bet_Type'] == 'tricomination':
                                all_tricomination_bets.append(b)
                except Exception as e:
    ...
    # Combine all bets
    all_bets = all_trifecta_bets + all_tricomination_bets

    if all_bets:
        bets_df = pd.DataFrame(all_bets)
    else:
        bets_df = pd.DataFrame(columns=['レースID', 'Boat1', 'Boat2', 'Boat3', 'Bet_Type', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout'])

    # 日付情報の抽出
    if not bets_df.empty:
        bets_df['Date'] = pd.to_datetime(bets_df['レースID'].str[:8], format='%Y%m%d')
    else:
        bets_df['Date'] = pd.to_datetime([])  # 空のDatetime列

    # 総投資額と総回収額を計算
    if not bets_df.empty:
        total_investment = bets_df['Bet_Amount'].sum()
    ...
    return bets_df, total_investment, total_payout, return_rate

def process_race_trifecta(race_id, race_odds, data_boats, total_capital, unit, prob_threshold):
    """
    三連単の賭け金額を決定し、回収金額を計算します。
    Args:
        race_id (str): レースID
        ...
        prob_threshold (float): この確率以下の組み合わせは賭けない
    Returns:
        list of dict or None: 賭け情報の辞書のリストまたはNone
    """
    boats_data = data_boats[data_boats['レースID'] == race_id]
    # if boats_data.empty or boats_data['会場'].values[0] in ['05', '06', '07', '08', '09', '11', '12', '18', '22', '23']:
    #     return None
    if boats_data.empty:
        return None
    ...
    # Assign race_id and bet type
    merged_df['レースID'] = race_id
    merged_df['Bet_Type'] = 'trifecta'

    # Select and reorder columns
    trifecta_bet = merged_df[['レースID', 'Boat1', 'Boat2', 'Boat3', 'Bet_Type', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout']]

    # Return as list of dictionaries
    return trifecta_bet.to_dict('records')

def process_race_tricomination(race_id, race_odds, data_boats, total_capital, unit, prob_threshold):
    """
    三連複の賭け金額を決定し、回収金額を計算します。
    Args:
        race_id (str): レースID
        race_odds (DataFrame): 該当レースの三連複オッズデータ
        data_boats (DataFrame): 各レースの各艇の確率データ
        total_capital (int): 総資金
        unit (int): 賭け金の単位（円）
        prob_threshold (float): この確率以下の組み合わせは賭けない
    ...
    # 全ての組み合わせ（三連複）
    tricomination_combinations = list(itertools.combinations(top_boats, 3))  # 4C3 = 4 combinations

    # Create a DataFrame of these combinations
    tricomination_df = pd.DataFrame(tricomination_combinations, columns=['Boat1', 'Boat2', 'Boat3'])

    # Sort boat numbers within each combination to ensure consistency
    tricomination_df = tricomination_df.apply(lambda row: sorted(row), axis=1, result_type='expand')
    tricomination_df.columns = ['Boat1', 'Boat2', 'Boat3']

    # Merge with race_odds to get odds
    merged_df = pd.merge(tricomination_df, race_odds, on=['Boat1', 'Boat2', 'Boat3'], how='left')

    # Drop combinations that do not have odds (if any)
    merged_df = merged_df.dropna(subset=['Odds'])

    # Calculate probability (order does not matter)
    merged_df['Probability'] = merged_df.apply(lambda row: calculate_trifecta_probability(  # Reuse the same probability function
        boats_probs.get(row['Boat1'], 0),
        boats_probs.get(row['Boat2'], 0),
        boats_probs.get(row['Boat3'], 0)
    ), axis=1)
    ...
    # Determine the actual combination (sorted)
    boats_actual = boats_data.sort_values(by='着')['艇番'].tolist()
    if len(boats_actual) < 3:
        actual_combo = ()
    else:
        actual_combo = tuple(sorted(boats_actual[:3]))

    # Calculate payout
    merged_df['Payout'] = merged_df.apply(
        lambda row: row['Bet_Amount'] * row['Odds'] if tuple(sorted([row['Boat1'], row['Boat2'], row['Boat3']])) == actual_combo else 0,
        axis=1
    )

    # Assign race_id and bet type
    merged_df['レースID'] = race_id
    merged_df['Bet_Type'] = 'tricomination'

    # Select and reorder columns
    tricomination_bet = merged_df[['レースID', 'Boat1', 'Boat2', 'Boat3', 'Bet_Type', 'Probability', 'Odds', 'EV', 'Bet_Amount', 'Payout']]

    # Return as list of dictionaries
    return tricomination_bet.to_dict('records')
...
def calculate_trifecta_probability(p1, p2, p3):
    ...
    if denominator1 <= 0 or denominator2 <= 0:
        return 0
    return p1 * p2 * p3  # Adjusted for tricomination as well
...
def plot_results(trifecta_bets, capital_evolution):
    ...
    if capital_evolution.empty or (capital_evolution.shape[0] == 1 and capital_evolution['Capital'].iloc[0] == 100000):
        ...
        plt.title("三連単・三連複の累積回収金額 vs 累積投資金額")
        plt.xlabel("日付")
        plt.ylabel("金額 (円)")
        plt.legend()
        plt.grid(True)
        plt.savefig('cumulative_return_trifecta_tricomination_today.png')
        plt.show()
        print("プロットが 'cumulative_return_trifecta_tricomination_today.png' として保存されました。")
        ...
        plt.tight_layout()
        plt.savefig('cumulative_return_by_date_trifecta_tricomination_today.png')
        plt.show()
        print("日付ごとの累積リターンの推移が 'cumulative_return_by_date_trifecta_tricomination_today.png' として保存されました。")
        ...
        if not trifecta_bets.empty:
            ...
            plt.ylabel("度数")
            plt.grid(True)
            plt.savefig('payout_distribution_trifecta_tricomination_today.png')
            plt.show()
            print("回収金額の分布が 'payout_distribution_trifecta_tricomination_today.png' として保存されました。")
            ...
            plt.ylabel("度数")
            plt.grid(True)
            plt.savefig('bet_amount_distribution_trifecta_tricomination_today.png')
            plt.show()
            print("賭け金額の分布が 'bet_amount_distribution_trifecta_tricomination_today.png' として保存されました。")
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig('investment_vs_payout_trifecta_tricomination_today.png')
            plt.show()
            print("累積投資金額と累積回収金額の比較が 'investment_vs_payout_trifecta_tricomination_today.png' として保存されました。")
            ...
            plt.savefig('cumulative_return_by_date_trifecta_tricomination_today.png')
            plt.show()
            print("日付を横軸とした累積収益の推移が 'cumulative_return_by_date_trifecta_tricomination_today.png' として保存されました。")
        else:
...
def correlation_analysis(trifecta_bets, data):
    ...
    plt.tight_layout()
    plt.savefig('correlation_heatmap_trifecta_tricomination_today.png')
    plt.show()
    print("相関ヒートマップを 'correlation_heatmap_trifecta_tricomination_today.png' として保存しました。")
...
def visualize_conditions(trifecta_bets, data):
    ...
    if 'weather' in merged_data.columns:
        ...
        plt.ylabel('回収率')
        plt.tight_layout()
        plt.savefig('return_rate_by_weather_trifecta_tricomination_today.png')
        plt.show()
        print("天候ごとの回収率の分布を 'return_rate_by_weather_trifecta_tricomination_today.png' として保存しました。")
    
    # 会場ごとの回収率の箱ひげ図
    if '会場コード' in merged_data.columns:
        plt.figure(figsize=(12, 6))
        ...
        plt.ylabel('回収率')
        plt.tight_layout()
        plt.savefig('return_rate_by_venue_trifecta_tricomination_today.png')
        plt.show()
        print("会場ごとの回収率の分布を 'return_rate_by_venue_trifecta_tricomination_today.png' として保存しました。")
