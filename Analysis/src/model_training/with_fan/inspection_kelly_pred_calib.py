...
def inspect_model():
    ...
    try:
        ...
        gbm_odds = lgb.Booster(model_file='boatrace_odds_model_calibrated.txt')
    ...
    try:
        ...
        lr_reg = joblib.load('lr_reg.pkl')  # ロジスティック回帰モデル
        iso_reg = joblib.load('iso_reg.pkl')  # アイソトニック回帰キャリブレーションモデル
    except Exception as e:
    ...
    feature_names_odds = pd.read_csv('feature_names_odds_calibrated.csv').squeeze().tolist()

    # 評価期間のデータ日付リスト作成
    month_folders = ['2410']
    date_files = {
        '2410': [f'2410{day:02d}' for day in range(23,24)],   # 241023
    }
    ...
    plt.show()
    print("プロットが 'prob_0_distribution_1min.png' として保存されました。")
    ...
    features_odds = ['会場', '艇番', '級別', '支部', '年齢', '体重', '全国勝率', '全国2連率',
                    ...
                    # 'win_odds1min', 'prob_0','win_odds_mean',
                    '前期能力指数', '今期能力指数', '平均スタートタイミング', '性別', '勝率', '複勝率', '優勝回数', '優出回数']

    target_odds = 'win_odds'

    data = data.dropna()

    X_odds = data[features_odds]
    y_odds = data[target_odds]
    ...
    X_processed_odds = X_processed_odds[feature_names_odds]  # 学習時の特徴量と順序を合わせる

    # 予測
    odds_pred = gbm_odds.predict(X_processed_odds)
    ...
    # # Reshape data to fit the linear model for isotonic regression
    ...
    # Calibrate the predictions using the loaded Isotonic Regression model
    ...
    filtered_data = data[data['diff'] > 1.5]
