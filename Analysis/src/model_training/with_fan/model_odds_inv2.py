...
def create_model():
    # 学習済みモデルの読み込み
    try:
        gbm = lgb_optuna.Booster(model_file='boatrace_model_first.txt')
    ...
    # データの読み込み（あなたのコードでデータを読み込んでください）
    # data = pd.read_csv('your_data.csv')
    # ここでは、データの前処理と特徴量エンジニアリングが完了していると仮定します。
    ...
    # --- データの分割 ---
    X_train, X_val, y_train, y_val, race_ids_train, race_ids_val, prob_0_train, prob_0_val = train_test_split(
        X_processed_multi, y_race, race_ids_array, prob_0_array, test_size=0.2, random_state=SEED)
    ...
    # --- 予測 ---
    y_pred_inv = model.predict(X_val)  # 逆数オッズの予測

    # 予測値を正の値にクリッピング（逆数オッズは正の値）
    y_pred_inv = np.maximum(y_pred_inv, 0.01)  # 最小値を0.01に設定（オッズの最大値100に対応）

    # --- 各レースごとにオッズを再計算 ---
    # 結果を格納するリスト
    y_pred_adjusted_list = []
    y_val_original_list = []
    race_ids_list_final = []
    boat_numbers_list = []
    prob_0_final_list = []
    ...
    for i in range(len(X_val)):
        race_id = race_ids_val[i]
        y_pred_inv_race = y_pred_inv[i]
        y_val_inv_race = y_val[i]
        prob_0_race = prob_0_val[i]

        # 元のオッズに戻す
        y_pred_race = 1 / y_pred_inv_race
        y_val_race = 1 / y_val_inv_race

        # オッズの範囲を調整
        max_odds = 100  # 最大オッズを100に設定
        min_inverse_odds = 1 / max_odds  # 最小逆数オッズは0.01
        y_pred_race = np.clip(y_pred_race, 1.0, max_odds)
        y_val_race = np.clip(y_val_race, 1.0, max_odds)

        # 各艇の勝率（prob_0）を取得
        probabilities = prob_0_race

        # 勝率の高い順に並べ替え
        sorted_indices = np.argsort(-probabilities)
        y_pred_race_sorted = y_pred_race[sorted_indices]
        y_val_race_sorted = y_val_race[sorted_indices]
        prob_0_race_sorted = probabilities[sorted_indices]

        # オッズの逆数の和を計算
        total_inverse_odds = 1.37931
        ...
        for j in range(5):
            ...
            pred_odds = y_pred_race[idx]
            pred_odds = np.clip(pred_odds, 1.0, max_odds)  # オッズは1以上、最大100
            adjusted_odds.append(pred_odds)
            sum_inverse_odds += 1 / pred_odds

        # 最後の一艇のオッズを計算
        remaining_inverse_odds = total_inverse_odds - sum_inverse_odds
        remaining_inverse_odds = np.clip(remaining_inverse_odds, min_inverse_odds, 1.0)  # 最小0.01, 最大1.0

        last_odds = 1 / remaining_inverse_odds
        last_odds = np.clip(last_odds, 1.0, max_odds)  # オッズは1以上、最大100
        adjusted_odds.append(last_odds)

        # 元の順序に戻す
        adjusted_odds_original_order = np.zeros(6)
        adjusted_odds_original_order[sorted_indices] = adjusted_odds

        y_pred_adjusted_list.extend(adjusted_odds_original_order)
        y_val_original_list.extend(y_val_race)
        race_ids_list_final.extend([race_id]*6)
        boat_numbers_list.extend(np.arange(1,7))
        prob_0_final_list.extend(prob_0_race)
        inverse_prob_0_list.extend(1 / prob_0_race)

    # --- 結果の整形 ---
    pred_df = pd.DataFrame({
        'レースID': race_ids_list_final,
        '艇番': boat_numbers_list,
        'predicted_odds': y_pred_adjusted_list,
        'true_odds': y_val_original_list,
        'prob_0': prob_0_final_list,
        'inverse_prob_0': inverse_prob_0_list
    })

    # --- 評価 ---
    # MAEの計算
    mae = mean_absolute_error(pred_df['true_odds'], pred_df['predicted_odds'])
    print(f'Mean Absolute Error on validation data: {mae:.4f}')

    # R²スコアの計算
    r2 = r2_score(pred_df['true_odds'], pred_df['predicted_odds'])
    print(f'R² Score on validation data: {r2:.4f}')

    # --- 可視化 ---
    # 1. 実際のオッズと予測オッズの散布図（全体）
    plt.figure(figsize=(8, 8))
    plt.scatter(pred_df['true_odds'], pred_df['predicted_odds'], alpha=0.5, label='予測オッズ')
    plt.scatter(pred_df['true_odds'], pred_df['inverse_prob_0'], alpha=0.5, label='1 / prob_0')
    plt.plot([pred_df['true_odds'].min(), pred_df['true_odds'].max()],
             [pred_df['true_odds'].min(), pred_df['true_odds'].max()], 'r--')
    plt.xlabel('実際のオッズ')
    plt.ylabel('オッズ')
    plt.title('実際のオッズと予測オッズ、および1 / prob_0の比較（全体）')
    plt.legend()
    plt.savefig('predicted_vs_true_overall_sequential.png')
    plt.show()
    ...
    for boat in range(1, 7):
        ...
        plt.scatter(boat_data['true_odds'], boat_data['predicted_odds'], alpha=0.5, label='予測オッズ')
        plt.scatter(boat_data['true_odds'], boat_data['inverse_prob_0'], alpha=0.5, label='1 / prob_0')
        plt.plot([boat_data['true_odds'].min(), boat_data['true_odds'].max()],
                 [boat_data['true_odds'].min(), boat_data['true_odds'].max()], 'r--')
        plt.xlabel('実際のオッズ')
        plt.ylabel('オッズ')
        plt.title(f'実際のオッズと予測オッズ、および1 / prob_0の比較（艇番 {boat}）')
        plt.legend()
        plt.savefig(f'predicted_vs_true_boat_{boat}_sequential.png')
        plt.show()

    # 3. 実際のオッズ、予測オッズ、1 / prob_0のヒストグラム（全体）
    ...
    plt.hist(pred_df['inverse_prob_0'], bins=50, alpha=0.5, label='1 / prob_0')
    plt.xlabel('オッズ')
    plt.ylabel('頻度')
    plt.title('実際のオッズ、予測オッズ、1 / prob_0の分布（全体）')
    plt.legend()
    plt.savefig('odds_distribution_overall_sequential.png')
    plt.show()
    ...
    plt.ylabel('頻度')
    plt.title('予測誤差の分布')
    plt.savefig('prediction_error_distribution_sequential.png')
    plt.show()
