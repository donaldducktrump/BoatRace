...
def inspect_model():
    ...
    date_files = {
        ...
        # '2403': [f'2403{day:02d}' for day in range(1, 15)],
    ...
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('prob_0_distribution_trifecta.png')
    plt.show()
    print("プロットが 'prob_0_distribution_trifecta.png' として保存されました。")
    ...
    # 結果をCSVに保存
    summary_df.to_csv('trifecta_summary_comparison.csv', index=False)
    print("結果の比較を 'trifecta_summary_comparison.csv' として保存しました。")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('revenue_per_date.png')
    plt.show()
    print("日付ごとの収益を 'revenue_per_date.png' として保存しました。")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('investment_per_date_histogram.png')
    plt.show()
    print("1日当たりの投資金額のヒストグラムを 'investment_per_date_histogram.png' として保存しました。")
    ...
    # 会場ごとの回収率をCSVに保存
    venue_summary.to_csv('return_rate_per_venue.csv', index=False)
    print("会場ごとの回収率を 'return_rate_per_venue.csv' として保存しました。")
    # ------------- 追加部分終了 -------------
...
def process_race(race_id, race_odds, data_boats, total_capital=100000, unit=100, prob_threshold=0.0, ev_subtract=1.4):
    ...
    """
    # Remove duplicates in race_odds
    race_odds = race_odds.drop_duplicates(subset=['Boat1', 'Boat2', 'Boat3'])
...
def calculate_trifecta_bets(trifecta_odds_df, data_boats, total_capital=100000, unit=100, prob_threshold=0.0, ev_subtract=1.4):
    ...
    # Remove duplicates in the odds DataFrame
    trifecta_odds_df = trifecta_odds_df.drop_duplicates(subset=['レースID', 'Boat1', 'Boat2', 'Boat3'])
...
def calculate_capital_evolution(trifecta_bets, initial_capital=100000):
    ...
    trifecta_bets['date'] = trifecta_bets['レースID'].str[:8]
    trifecta_bets['date'] = pd.to_datetime(trifecta_bets['date'], format='%Y%m%d')
...
def plot_cumulative_returns(cumulative_returns):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('cumulative_return_comparison.png')
    plt.show()
    print("資金の推移を 'cumulative_return_comparison.png' として保存しました。")
