...
def add_probability_column(data):
    # グループごとにpred_scoreをソフトマックス変換して確率を計算
    data['pred_prob'] = data.groupby('レースID')['pred_score'].apply(lambda x: np.exp(x) / np.exp(x).sum())
    return data
...
def inspect_model():
    ...
    try:
        ...
        ranker = lgb.Booster(model_file='boatrace_ranker_model.txt')
    ...
    date_files = {
        ...
        '2410': [f'2410{day:02d}' for day in range(16,28,2)],   # 241001 - 241008
    }
    ...
    print(data[['レースID', '艇番', 'prob_0','prob_1', '順位_1_確率', '順位_2_確率', '順位_3_確率', '順位_4_確率', '順位_5_確率', '順位_6_確率']].head())
    # 'prob_0' の分布をプロット
    ...
    data.loc[data['艇番'] == 1, 'odds_pred'] = data.loc[data['艇番'] == 1, 'odds_pred'] 
    ...
    data['bet_amount'] = np.where(data['順位_1_確率'] < 0.2, 0, data['bet_amount'])  # 確率が低い場合は掛け金を0にする
    ...
    filtered_data = data[data['艇番'] == 2]
    print(filtered_data[['レースID','艇番','prob_0','win_odds1min','win_odds','odds_pred','bet_amount', 'payout']].head(50))
    
    plt.figure(figsize=(10, 6))
    plt.hist(filtered_data['odds_pred'], bins=500, color='blue', edgecolor='black', alpha=0.7)
    plt.title("予測オッズ 'odds_pred' の分布")
    plt.xlabel("予測オッズ (odds_pred)")
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('odds_pred_distribution_1min.png')
    plt.show()
    print("プロットが 'odds_pred_distribution.png' として保存されました。")
    ...
    plt.ylabel("度数")
    plt.grid(True)
    plt.savefig('odds_actual_distribution_1min.png')
